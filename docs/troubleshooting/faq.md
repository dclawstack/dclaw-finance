...
### How do I update DClaw Finance?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp finance --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Finance without Kubernetes?

Yes, for local development:

```bash
cd dclaw-finance/frontend && npm run dev
cd dclaw-finance/backend && uvicorn dclaw_finance.main:app --reload
```
